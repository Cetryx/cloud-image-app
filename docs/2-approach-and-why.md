...
Another part of my approach was setting up a remote Terraform state. At the beginning, Terraform used a local state file. After the storage account and a separate `tfstate` container were created, I configured the AzureRM backend and migrated the state into Azure Blob Storage. This means the Terraform state is now stored in Azure instead of only on my local machine. I chose this because remote state is more suitable for a DevOps workflow and later pipeline usage.

For the remote state, I created a separate private container named `tfstate`. The state file is stored there as `cloud-image-app.tfstate`. I also assigned the `Storage Blob Data Contributor` role to my current Azure user, so Terraform can read and write the state through Azure RBAC. The backend uses Azure AD authentication instead of storing a storage account access key in the backend configuration.
